...
namespace BubbleSystem2
{
    public class BubbleSystemManager : MonoBehaviour
    {
        ...
        private Dictionary<Tutor.TutorEnum, BubbleSystemData> tutorNextData = new Dictionary<Tutor.TutorEnum, BubbleSystemData>();
        ...
        private void SetData(ref BubbleSystemData data)
        {
            if (!tutorNextData.ContainsKey(data.tutor.Get())) return;
            BubbleSystemData newData = tutorNextData[data.tutor.Get()];
            if (newData.IsCleared()) return;
            data = newData;
        }

        public void AddTutorNextData(BubbleSystemData data)
        {
            BubbleSystemUtility.AddToDictionary(ref tutorNextData, data.tutor.Get(), data);
        }
    }
}
