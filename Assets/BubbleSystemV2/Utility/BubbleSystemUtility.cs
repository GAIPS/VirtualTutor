...
namespace BubbleSystem2
{
    ...
    {
        public static KeyValuePair<Emotion, float> GetHighestEmotion(Dictionary<Emotion, float> emotions)
        {
            Emotion highestEmotion = emotions.Aggregate((l, r) => l.Value > r.Value ? l : r).Key;
            float intensity = emotions[highestEmotion];

            return new KeyValuePair<Emotion, float>(highestEmotion, intensity);
        }

        public static float GetEmotionsSum(Dictionary<Emotion, float> emotions)
        {
        ...
        public static Color32 MixColors(Dictionary<Emotion, float> emotions)
        {
            ...
            {
                color = Color.Lerp(color, (Color)DefaultData.Instance.GetColor(emotion.Get()), emotions[emotion]);
            }
